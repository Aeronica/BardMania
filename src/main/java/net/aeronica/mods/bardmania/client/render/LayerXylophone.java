...
public class LayerXylophone implements LayerRenderer<EntityLivingBase>
{
    private static final LayerXylophone LAYER_XYLOPHONE = new LayerXylophone();
    ...
    public void doRenderLayer(EntityLivingBase entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        if (!entitylivingbaseIn.isInvisible() && (entitylivingbaseIn.getHeldItemMainhand().getItem() instanceof ItemHandHeld))
        {
            ...
            if (instrument.general.holdType.equals(HoldType.TWO_HANDED_DRUM) && instrument.id.equalsIgnoreCase("xylophone"))
            {
    ...
    public static void addLayer()
    {
        ...
        try
        {
            Minecraft.getMinecraft().getRenderManager().getSkinMap().entrySet().forEach(e -> {
                e.getValue().addLayer(LAYER_XYLOPHONE);
                isAdded = true;
                ModLogger.info("LayerXylophone added %s", e.toString());
            });
        } catch (Exception e)
        {
            ModLogger.error(e);
